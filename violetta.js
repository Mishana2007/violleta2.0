...
const tests = {
    ...
    test4: {
        ...
        questions: [
            ...
            'Мысли о смерти',
            'Переедание',
            'Ощущение неловкости, когда люди наблюдают за вами или говорят о вас',
            'То, что у вас в голове чужие мысли',
            'Импульсы причинять телесные повреждения или вред кому-либо',
            'Бессонница по утрам',
            'Потребность повторять действия: прикасаться, мыться, пересчитывать и т.д.',
            'Беспокойный и тревожный сон',
            'Импульсы ломать или крушить что-нибудь',
            'Наличие у вас идей или верований, которых не разделяют другие',
            'Чрезмерная застенчивость при общении с другими',
            'Чувство неловкости в людных местах (магазинах, кинотеатрах)',
            'Чувство, что все, что бы вы ни делали, требует больших усилий',
            ...
            'То, что вы кричите и швыряетесь вещами',
            ...
            'Мысли, что вы должны быть наказаны за ваши грехи',
  ...
  async function handleTest4Answer(chatId, questionIndex, optionIndex) {
    try {
        ...
        if (!userAnswers.get(chatId).test4) {
            userAnswers.get(chatId).test4 = new Array(90).fill(0);
        }

        userAnswers.get(chatId).test4[questionIndex] = parseInt(optionIndex);

        // Проверяем, все ли вопросы отвечены
        if (questionIndex + 1 < 90) {
...
async function analyzeTest4Results(answers) {
    try {
        ...
        // Вычисление значений по каждой шкале
        const scaleScores = {};
        for (const [scale, data] of Object.entries(SCL90Scales)) {
            const scaleAnswers = data.questions.map(q => answers[q] || 0);
            const sum = scaleAnswers.reduce((acc, val) => acc + val, 0);
            scaleScores[scale] = {
                raw: sum,
                average: (sum / data.questions.length).toFixed(2)
            };
        }

        // Вычисление обобщенных индексов
        const allAnswers = answers.filter(a => a !== undefined && a !== null);
        const nonZeroAnswers = allAnswers.filter(a => a > 0);
        ...
        // PSDI (Индекс личного симптоматического дистресса)
        const PSDI = PST > 0 ? (totalSum / PST).toFixed(2) : 0;
        ...
        console.error('Ошибка при анализе результатов теста 4:', error);
        throw error;
    }
}
